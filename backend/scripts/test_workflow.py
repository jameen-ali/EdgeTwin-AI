...
def test_workflow():
    ...
    for role, creds in users.items():
        ...
        if resp.status_code == 200:
            ...
            print(f"❌ {role} login failed: {resp.text}")
            return
    ...
    if resp.status_code == 200:
        print("✅ High temp reading submitted.")
    else:
        print(f"❌ Failed to submit reading: {resp.text}")
    ...
    print(f"✅ Found pending alert {alert_id}")
    ...
    if resp.status_code == 200:
        ...
        print(f"✅ Ticket {ticket_id} created and assigned.")
    else:
    ...
    if resp.status_code == 200:
        print("✅ Ticket marked in_progress.")
    else:
        print(f"❌ Failed to update ticket status: {resp.text}")
    ...
    if resp.status_code == 200:
        print("✅ Ticket marked repaired.")
    else:
        print(f"❌ Failed to complete ticket: {resp.text}")
    ...
    if resp.status_code == 200:
        print("✅ Ticket closed by manager.")
    else:
        print(f"❌ Failed to close ticket: {resp.text}")
        
    resp = requests.get(f"{BASE_URL}/admin/audit", headers=headers_admin)
    if resp.status_code == 200:
        print(f"✅ Audit logs accessible. (Count: {len(resp.json())})")
    else:
        print("❌ Audit log fetch failed.")
